    ...
    <section id="about" className="py-20 bg-black/40 backdrop-blur-sm">
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div
            ref={statsRef}
            className={`grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 transition-all duration-1000 ease-out ${isStatsVisible
          ...
          >
            {/* Technologies Card */}
            <div
              className={`transition-all duration-700 ease-out ${isStatsVisible
                  ? 'opacity-100 translate-y-0'
          ...
          <div className="mb-16">
            ...
            <div className="flex justify-center mb-12 px-2 sm:px-4">
              <div className="flex flex-wrap sm:flex-nowrap justify-center gap-2  bg-gradient-to-r from-black/30 via-black/20 to-black/30 backdrop-blur-md rounded-2xl p-1.5 border border-white/10 shadow-lg shadow-blue-500/20 max-w-full overflow-hidden w-full max-w-sm sm:w-auto sm:max-w-none">
                {tabCategories.map((tab) => (
                  <button
                    key={tab.key}
                    onClick={() => setActiveTab(tab.key)}
                    className={`flex items-center justify-center gap-1 sm:gap-1 md:gap-2 lg:gap-3 px-6 sm:px-3 md:px-4 lg:px-6 py-3 md:py-4 rounded-xl transition-colors duration-500 exo2-regular relative overflow-hidden group border whitespace-nowrap min-w-fit flex-1 sm:flex-none ${activeTab === tab.key
                  ...
                  >
                    ...
                    <span className={`text-base sm:text-lg md:text-xl transition-transform duration-300 ${activeTab === tab.key
                        ? 'scale-110 filter '
                    ...
                    <span className={`hidden sm:inline font-light text-xs sm:text-sm md:text-base tracking-wide transition-colors duration-300 ${activeTab === tab.key
                        ? 'font-medium'
                        : ''
                      }`}>
                      {tab.label}
            ...
            <div
              ref={techStackRef}
              className={`min-h-[300px] sm:min-h-[350px] md:min-h-[400px] px-2 sm:px-4 md:px-0 transition-all duration-1000 ease-out ${isTechStackVisible
                  ? 'opacity-100 translate-y-0'
                  : 'opacity-0 translate-y-10'
                }`}
            >
          ...
          <div>
            ...
            <div
              ref={experienceRef}
              className={`relative p-8 bg-gradient-to-br from-white/5 via-white/2 to-transparent backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl shadow-black/30 hover:shadow-cyan-500/10 transition-all duration-1000 overflow-hidden ${isExperienceVisible
                  ? 'opacity-100 translate-y-0'
                  : 'opacity-0 translate-y-10'
                }`}
            >
              ...
              <div className="relative z-10">
                {experience.map((exp, index) => (
                  <div
                    key={exp.id}
                    className={`relative flex gap-6 pb-12 last:pb-0 group transition-all duration-700 ease-out ${isExperienceVisible
